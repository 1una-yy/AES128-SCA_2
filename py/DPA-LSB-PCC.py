...
for i in range(trace_num):
    ...
    t_sum += t[i]
    t2_sum += t[i] ** 2
    std_t = np.sqrt(t2_sum - (t_sum ** 2) / n) + 1e-40       # 加一個很小的數(10的負40次方)，以避免跳出runtimewarning的警告
    for b in range(16):                             # 共 16 bytes
        ...
        std_h = np.sqrt(h2_sum[b] - (h_sum[b] ** 2) / n) + 1e-40 # 加一個很小的數(10的負40次方)，以避免跳出runtimewarning的警告
        r[b] = (h_t_sum[b] - np.outer(h_sum[b], t_sum) / n) / np.outer(std_h, std_t) # 計算相關係數值
...
for b in range(16):
    ...
    true_key = k[b, b]   # 真實key
    for k_guess in range(256):
        if k_guess == true_key:
            axes[b // 4, b % 4].plot(max_coef_per_row[:, b, k_guess], color='red')
        else:
            axes[b // 4, b % 4].plot(max_coef_per_row[:, b, k_guess], color='gray', alpha=0.5)
