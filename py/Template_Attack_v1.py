...
poi_num = 10                                     # 取 SNR 陣列前 10 高的數值

poi_index = np.argsort(SNR, axis=1)[:, -poi_num:]# 將SNR每一列的數值由小到大排序後，取得後poi_num筆的索引值，大小為(16, poi_num)
Guess_Key = np.zeros((16), dtype=int)

# print(poi_index)
print("計算開始")
for b in range(16):                              # 16bytes，跑16次
    target_poi = target_t[:, poi_index[b]]       # poi_index每一列(byte)代表最大SNR的index，找出traget_traces對應index的數值
    template_poi = template_t[:, poi_index[b]]   # 
    mean = np.zeros((9, poi_num))                # 分9群，共取poi_num個點
    covariance = np.zeros((9, poi_num, poi_num)) # 分9群，大小為poi_num * poi_num的矩陣
    
    for i in range(9):                           # 根據假設的中間值矩陣(target_h)分成9群
        # 根據假設的中間值矩陣的元素值(0~8)，如果 HW=0 就將trace_t的那十個點丟到g陣列中，以此類推
        g = template_poi[template_h[:, b]==i, :]
        mean[i] = np.mean(g, axis=0)             # 因為g是大小為(x, 10)的二維陣列，將這些陣列取平均，即對每一個陣列對應的元素取平均
        covariance[i] = np.cov(g, rowvar=False)  # 計算g的共變異數，rowver=False代表以行為主，各行之間的共變異數，故大小為10*10，如果rowvar=True則為以列為主
    
    hw_log_prob = np.zeros(9)
    key_log_prob = np.zeros(256)
    
#     target_trace_num
    for i in range(100):                          # 一次 load 一條 trace
        for j in range(9):                       # load 一條trace後，跟template的 9 群做比較，算maximum-likehood
            hw_log_prob[j] = np.log(multivariate_normal.pdf(target_poi[i], mean=mean[j], cov=covariance[j], allow_singular=True)/hw_combinations[j]+1e-40)
            # print(i, j, hw_log_prob[j])
        key_log_prob += hw_log_prob[hw_list[AES_Sbox[np.arange(256)^target_p[i, b]]]]# 因為取log後，由相乘變成相加
    Guess_Key[b] = np.argmax(key_log_prob)    
...
for b in range(16):
    ...
    template_poi = template_t[:, poi_index[b]]
    mean = np.zeros((256, poi_num))
    covariance = np.zeros((256, poi_num, poi_num))
    
    for i in range(256):
        g = template_poi[template_y[:, b]==i, :]
        mean[i] = np.mean(g, axis=0)
        covariance[i] = np.cov(g, rowvar=False)
...
#     target_trace_num
    for i in range(100):
        for j in range(256):
            temp[j] = np.log(multivariate_normal.pdf(target_poi[i], mean=mean[j], cov=covariance[j], allow_singular=True)+1e-40)
        key_log_prob += temp[AES_Sbox[np.arange(256)^target_p[i, b]]]
    Guess_Key[b] = np.argmax(key_log_prob)
    print(b, ' ', end='')
print('')
print(Guess_Key)
